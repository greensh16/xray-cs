...
- **First-class `dask_setup` 2.x support.** Importing `dask_setup` now activates
  the Dask rule domain even when `dask` is not imported directly, including
  aliased imports and cross-cell notebook usage. `setup_dask_client` and
  `DaskClientContext` workload selection are resolved when literal; GPU mode
  now counts as GPU intent for JOB004.
- **DK011 — `dask_setup` I/O topology with NetCDF/HDF5.** Warns when the
  package's one-process, many-thread `workload_type="io"` topology is paired
  with a backend whose process-wide lock serialises those threads. Named
  profiles, config objects, `**kwargs`, and dynamic workload values stay quiet
  when xray cannot prove the topology.

### Fixed

- **XR008 no longer gives conflicting `dask_setup` advice.** For a statically
  resolved I/O topology and NetCDF/HDF5 input, it does not recommend or
  auto-insert `parallel=True`; DK011 instead recommends the package's CPU
  topology. The cache format is v3 so older cached diagnostics are rebuilt.
...
[Unreleased]: https://github.com/greensh16/xray-cs/compare/v1.3.1...HEAD
[1.3.1]: https://github.com/greensh16/xray-cs/compare/v1.3.0...v1.3.1
